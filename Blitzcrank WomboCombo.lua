...
function OnUnload()
	PrintFloatText(myHero,2,"Blitzcrank WomboCombo UnLoaded!")
end
function LoadMenu()
	Config = scriptConfig("Blitzcrank WomboCombo", "Blitzcrank WomboCombo")
	...
	PrintFloatText(myHero,2,"Blitzcrank WomboCombo Loaded!")
end
...
function LoadVIPPrediction()
	tpQ = TargetPredictionVIP(rangeQ, 1800, 0.5, 70)
end
function LoadCollisionValues()
	qcol = Collision(rangeQ, 1800, 0.5, 70)
end
	...
	if ValidTarget(target) then
		if GetDistance(target) <= rangeQ and QREADY then
			...
			if QPos and GetDistance(QPos)<=rangeQ then
				if not qcol:GetMinionCollision(myHero, QPos) then
