...
function OnUnload()
	PrintFloatText(myHero,2,"Lux WomboCombo UnLoaded!")
end
function LoadMenu()
	Config = scriptConfig("Lux WomboCombo ", "Lux WomboCombo")
	...
	Config:addParam("mindmgpercent", "Min dmg percent", SCRIPT_PARAM_SLICE, 10, 0, 100, 0)
	...
	PrintFloatText(myHero,2,"Lux WomboCombo Loaded!")
end
...
function LoadSkillRanges()
	rangeQ = 1250
	rangeW = 1175
	rangeE = 1175 -- radius is 250
	rangeR = 3000
	killRange = 1250
end
function LoadVIPPrediction()
	tpQ = Prodict(_Q, rangeQ, 1200, 0.5, nil, myHero, 
                function(unit, pos, castspell) 
                    if GetDistance(pos) < rangeQ and myHero:CanUseSpell(_Q) == READY then 
						CastSpell(_Q, pos.x, pos.z)
                    end 
                end)
	tpE = Prodict(_E, rangeE, 1300, 0.5, nil, myHero, 
                function(unit, pos, castspell) 
                    if GetDistance(pos) < rangeE and myHero:CanUseSpell(_E) == READY then 
						CastSpell(_E, pos.x, pos.z)
                    end 
                end)
	tpR = Prodict(_R, rangeR, 3000, 0.25, nil, myHero, 
                function(unit, pos, castspell) 
                    if GetDistance(pos) < rangeR and myHero:CanUseSpell(_R) == READY then 
						CastSpell(_R, pos.x, pos.z)
                    end 
                end)
end
function LoadMinions()
	enemyMinion = minionManager(MINION_ENEMY, rangeQ+200, player, MINION_SORT_HEALTH_ASC)
...
function LoadAllies()
	for i = 1, heroManager.iCount do
		...
		if hero.team == player.team then
			Config:addParam("teammateshield"..i, "Shield "..hero.charName, SCRIPT_PARAM_ONOFF, true)
		end
	end
end
	...
	for i = 1, enemyHerosCount do
		...
		if ValidTarget(Enemy) then
			...
			if killHim >= currentTarget.health and killMana<= myHero.mana then
				if GetDistance(currentTarget) <= rangeR and RREADY and GetDistance(currentTarget)>rangeQ then
					killRange = rangeR
				end
			...
			if usingE == true then
				if next(Eobj)~=nil then
					for i, obj in pairs(Eobj) do
						if obj.valid then
							if GetDistance(obj, Enemy)<=250 and GetDistance(obj, Enemy)>=240 then
								CastSpell(_E)
							end
						end
					end
			...
			if RREADY then
				local QPos = {x = myHero.visionPos .x, y = myHero.visionPos .y, z = myHero.visionPos .z}
				local HeroPos = Vector(myHero.x, myHero.y, myHero.z)
				local Pos = HeroPos +(HeroPos -QPos)*(-3000/GetDistance(QPos))
				local dist = GetDistance(myHero, Pos)
				
				if GetDistance(Enemy, myHero) < dist and GetDistance(Enemy, Pos) < dist then
					local _, p2, isOnLineSegment = VectorPointProjectionOnLineSegment(myHero, Pos, Enemy)
					if isOnLineSegment and GetDistance(Enemy, p2)<= 90 then
						EnemyCanBeUlted = EnemyCanBeUlted + 1
					end
				end
			end
		else
...
function OnGainBuff (unit, buff)
	if unit.isMe and unit.valid then
		if buff.name == "LuxLightStrikeKugel" then
			usingE = true
		end
	end
end
...
function CastQ(target)
	...
	if ValidTarget(target) then
		if GetDistance(target) <= rangeQ then
			enemyMinion:update()
			local minionCount = 0
			local dist = GetDistance(myHero, QPos)
			for j, minion in pairs(enemyMinion.objects) do
				if minion.valid then
					if GetDistance(minion, myHero) < dist and GetDistance(minion, QPos) < dist then
						local _, p2, isOnLineSegment = VectorPointProjectionOnLineSegment(myHero, target, minion)
						if isOnLineSegment and GetDistance(minion, p2)<= 110 then
...
function CastE(target)
	...
	if ValidTarget(target) then
		if GetDistance(target)<=rangeE and usingE == false then
			tpE:EnableTarget(target, true)
		end
	else
		return
	end
...
function OnProcessSpell(unit, spell)
	...
	if ValidTarget(unit) and unit.type == "obj_AI_Hero" then
		...
		if WREADY then
			for i=1, heroManager.iCount do
				allyTarget = heroManager:GetHero(i)
				if allyTarget.team == player.team and not allyTarget.dead then
					if GetDistance(allyTarget, myHero)<=rangeW and Config["teammateshield"..i] then
						...
						if spelltype == "BAttack" or spelltype == "CAttack" then
							if GetInventoryHaveItem(3186,unit) then 
								onhitdmg = getDmg("KITAES",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3114,unit) then 
								onhitdmg = onhitdmg+getDmg("MALADY",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3091,unit) then 
								onhitdmg = onhitdmg+getDmg("WITSEND",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3057,unit) then 
								onhitdmg = onhitdmg+getDmg("SHEEN",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3078,unit) then 
								onhitdmg = onhitdmg+getDmg("TRINITY",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3100,unit) then 
								onhitdmg = onhitdmg+getDmg("LICHBANE",allyTarget,unit)
							end
							if GetInventoryHaveItem(3025,unit) then 
								onhitdmg = onhitdmg+getDmg("ICEBORN",allyTarget,unit)
							end
							if GetInventoryHaveItem(3087,unit) then 
								onhitdmg = onhitdmg+getDmg("STATIKK",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3153,unit) then 
								onhitdmg = onhitdmg+getDmg("RUINEDKING",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3042,unit) then 
							 onhitdmg = onhitdmg+getDmg("MURAMANA",allyTarget,unit) 
							end
							if GetInventoryHaveItem(3209,unit) then 
								onhittdmg = getDmg("SPIRITLIZARD",allyTarget,unit) 
							end
						...
						elseif spelltype == "Q" or spelltype == "W" or spelltype == "E" or spelltype == "R" or spelltype == "P" or spelltype == "QM" or spelltype == "WM" or spelltype == "EM" then
							if GetInventoryHaveItem(3151,unit) then 
								onhitspelldmg = getDmg("LIANDRYS",allyTarget,unit)
							end
							if GetInventoryHaveItem(3188,unit) then 
								onhitspelldmg = getDmg("BLACKFIRE",allyTarget,unit)
							end
							if GetInventoryHaveItem(3209,unit) then 
								onhitspelltdmg = getDmg("SPIRITLIZARD",allyTarget,unit)
							end
							if GetInventoryHaveItem(3042,unit) then 
								muramanadmg = getDmg("MURAMANA",unit,allyTarget)
							end
							...
							skilldamage = getDmg(spelltype,allyTarget,unit,3,spellLevel)
						...
						elseif spellName:find("SummonerDot") then
							skilldamage = getDmg("IGNITE",allyTarget,unit)
						end
						if shottype == 0 then hitchampion = checkhitaoe(unit, P2, 80, allyTarget, 0)
						elseif shottype == 1 then 
							hitchampion = checkhitlinepass(unit, P2, radius, maxdistance, allyTarget, 50)
						elseif shottype == 2 then 
							hitchampion = checkhitlinepoint(unit, P2, radius, allyTarget, 50)
						elseif shottype == 3 then 
							hitchampion = checkhitaoe(unit, P2, radius, allyTarget, 50)
						elseif shottype == 4 then 
							hitchampion = checkhitcone(unit, P2, radius, maxdistance, allyTarget, 50)
						elseif shottype == 5 then 
							hitchampion = checkhitwall(unit, P2, radius, maxdistance, allyTarget, 50)
						elseif shottype == 6 then 
							hitchampion = checkhitlinepass(unit, P2, radius, maxdistance, allyTarget, 50) or checkhitlinepass(unit, Vector(unit)*2-P2, radius, maxdistance, allyTarget, 50)
						end
						local dmgpercent = skilldamage*100/allyTarget.health
						local dmgneeded = dmgpercent >= Config.mindmgpercent
						if hitchampion then
							if dmgneeded then

								CastSpell(_W, allyTarget.x, allyTarget.z)
							end
						end
					end
				end
...
function OnCreateObj(obj)	
	if obj.name:find("LuxLightstrike_tar_green.troy") then
		table.insert(Eobj, obj)
	end
end
function removeEobj()
	if next(Eobj)~=nil then
