...
function LoadCollisionValues()
	ecol = Collision(rangeE, 1200, 0.25, 60)
end
...
function LoadVIPPrediction()
	--q rangeQ, 1100, 0.5
	--e rangeE, 1250, 0.5
    tpQ = Prodict(_Q, rangeQ, 1100, 0.5, nil, myHero, 
                function(unit, pos, castspell) 
                    if GetDistance(pos) < rangeQ and myHero:CanUseSpell(_Q) == READY then 
						CastSpell(_Q, pos.x, pos.z)
                    end 
                end)           	
    tpE = Prodict(_E, rangeE, 1250, 0.5, nil, myHero,
                function(unit, pos, castspell) 
                    if GetDistanceSqr(unit) < castspell.RangeSqr and myHero:CanUseSpell(_E) == READY then 
...
function Target()
	...
	for i = 1, enemyHerosCount do
		...
		if ValidTarget(Enemy) then
			...
			if RREADY and not usingUlt then ultStacks = 3 end
			if ultStacks == 3 then
				rdmg = rdmg*3
